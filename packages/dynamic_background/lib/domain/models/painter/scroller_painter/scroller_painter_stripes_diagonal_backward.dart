...
class ScrollerPainterStripesDiagonalBackward extends ScrollerPainter {
  /// Creates a [ScrollerPainterStripesDiagonalBackward] object.
  ///
  /// This is the painter for animating stripes moving around the screen.
  ScrollerPainterStripesDiagonalBackward({
    required super.animation,
    required super.data,
  }) {
    if (!identical(data.shape, ScrollerShape.stripesDiagonalBackward)) {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (double i = 0.0 - (size.longestSide + shapeWidth + spaceBetweenShapes);
      ...
      if (identical(data.direction, ScrollDirection.left2Right) ||
          identical(data.direction, ScrollDirection.bottom2Top)) {
        tempXOffset = i +
            animation.value *
                (size.longestSide +
                    size.width +
      ...
      final Offset topLeft = Offset(xOffset, 0.0);
      final Offset topRight = Offset(xOffset + shapeWidth, 0.0);
      final Offset bottomRight = Offset(
        xOffset + shapeWidth + size.height,
        size.height,
      );
      final Offset bottomLeft = Offset(xOffset + size.height, size.height);
