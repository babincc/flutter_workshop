...
class DebugLog {
  ...
  static void out(String className, String methodName, String? message,
      {bool isImportant = false}) {
    assert(className.isNotEmpty, "You must enter a class name!");
    assert(methodName.isNotEmpty, "You must enter a method name!");

    // Pad the message if necessary.
    if (isImportant) {
      _printMessagePadding(className, methodName, padding: 3);
    }
    ...
    // Pad the message if necessary.
    if (isImportant) {
      _printMessagePadding(className, methodName,
          padding: 3, isPrePadding: false);
    }
  }
  ...
  /// before the error message; likewise, it will be `false` if it is to be
  /// printed after the error message.
  static void _printMessagePadding(String className, String methodName,
      {int padding = 1, bool isPrePadding = true}) {
